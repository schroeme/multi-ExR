...
res_5xFAD=[];
for jj = 1:5
    temp = data(jj).data.int_signal';%change this to pull different variables
...
for fidx = 1:length(ROIs)
    thresh(fidx).roiname = ROIs{fidx};
    thresh(fidx).thresh = determine_intensity_threshold_5xFAD(params,ROIs{fidx},syn_channels);
end
...
params.mask_abeta=1;
params.doplot=1;
params.morph_rad=100;

params.thresh_method = 'absolute';
params.thresholds = [55 %~rounded averages as determined above
80
40
50];
...
for fidx = 1:length(ROIs)
    data(fidx).roiname = ROIs{fidx};
    [data(fidx).data] = analyze_synapses_5xFAD(params,ROIs{fidx},syn_channels);
end
