...
parent = "A:/Jinyoung/2024.4.14 multiExR SST-GFP + PV/"; //parent folder where raw microscope images are stored (e.g. in .nd2 format)
parentout = "A:/Margaret/mExR/2024.04_SST-GFP_PV/preprocessed/round1_noSMIGFAP/"; //where to save the images
...
roundList = newArray("R1 wo SMI GFAP/");
					//"R2/");
					//"R3/",
					//"R4/",
					//"R5/",
...
for (m = 0; m < roundList.length; m++){
	for (i = 0; i < regList.length; i++){
	    for (j = 0; j < sampleList.length; j++){
	    	...
	    	folder = parent+roundList[m]+regList[i]+sampleList[j]+"40x/";
	    	print(folder);
		    	...
		    	if (endsWith(imgList[k], ".nd2")){ //CHANGE HERE for your filetype
		    		if (indexOf(imgList[k],"Max") < 0){ //CHANGE HERE: provide a string that is ONLY in the images you're interested in
		    			...
		    			// you can also skip this extra for-loop
		    				preprocessImage(imgList[k],folder,reg,sample,roundno);
		    			//}
					}
				}
	    	}
...
function preprocessImage(imageFile,folder,reg,sample,roundno)
{
	...
	roundid = "round002"; //hard code this for no GFAP/no SMI round 1
	newname = sample + "-" + reg + '-' + roino + "_" + roundid;
