...
parent = "A:/Jinyoung/2023.11.29 Stg rescue TRN multiExR/"; //parent folder where raw microscope images are stored (e.g. in .nd2 format)
parentout = "A:/Margaret/mExR/2023.11_StgRes_TRN/preprocessed/R1_noSMI_noGFAP/"; //where to save the images
...
roundList = newArray("R1 wo SMI GFAP/"); //round folders - again, will need to be modified depending on how you saved your data
...
for (m = 0; m < roundList.length; m++){
	for (i = 0; i < condList.length; i++){
	    for (j = 0; j < sampleList.length; j++){
	    	...
	    	folder = parent+roundList[m]+condList[i]+sampleList[j];
	    	//print(folder);
