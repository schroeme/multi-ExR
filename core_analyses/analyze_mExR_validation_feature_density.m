...
if params.regsummed
    for rridx = 1:nrounds %loop through each round
        for chidx = 1:nchannels %loop through each channel
            ...
            masksummed = or(mask,masksummed);
        end

        CC = bwconncomp(masksummed,26);%find all connected components in the filtered
        nobjects = CC.NumObjects;
    
        cc_volumes = regionprops3(CC,'Volume');
        cc_volumes=cc_volumes.Volume;
        
        nobjects(rridx) = nobjects;
        volume_temp = nnz(masksummed(:));
        volume(rridx) = volume_temp * params.vol_converter;
        volume_frac(rridx) = volume_temp/numel(masksummed);
        min_vol(rridx) = min(cc_volumes)/volume_temp;
        max_vol(rridx) = max(cc_volumes)/volume_temp;

        figure()
        imagesc(max(masksummed,[],3))
        colormap gray

        if params.writemask %save down the mask
            filename = [params.savefolder fov '_' 'round00' num2str(rridx) '_summedMask_MIP.tif'];
            imwrite(uint8(max(masksummed,[],3)),filename)
        end
