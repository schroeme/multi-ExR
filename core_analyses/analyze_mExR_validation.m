function [SNR_overall,SNR_synapses,nsynapses] = analyze_mExR_validation(fov,params)
...
for rridx = 1:nrounds
    for chidx = 1:nchannels
        if ~strcmp(num2str(chidx),morph_channel)
            ...
            fnames = dir([parentfolder '*' fov '*' 'round*' num2str(rridx) '*ch0' num2str(chidx) '*.tif']); %binary images
